...
df = pd.DataFrame(columns=['title', 'adress', 'metro_st', 'ap_info', 'text_sub', 'apart_feat',
...
def get_phone_numbers(card_page):
  ...
  if card_page.find(class_="a10a3f92e9--container--35iPF"):
    return card_page.find(class_="a10a3f92e9--container--35iPF").get_text().replace('Показать телефон', '').replace('Написать сообщение', '').split('+')
  return "NAN"
...
def create_full_data_list(card_page):
...
for i in page_list:
  ...
  for idx, val in enumerate(ads_url):
    ...
    web_ad_page = get_page_url(ad_url)
    ad_page = get_html_text_from_page(web_page)
    index = last_row + idx
    df.loc[index] = create_full_data_list(ad_page)
    print(index, ad_url)
